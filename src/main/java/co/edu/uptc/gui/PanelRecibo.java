...
public class PanelRecibo extends JDialog {
	...
	for (ProductoCompra productoCompra : recibo.getListaProductosComprados()) {
	    ...
	    tableModel.addRow(
		    new Object[] { isbn, producto, format.format(precioUnitario), cantidad, format.format(subtotal) });
	}

	tableModel
		.addRow(new Object[] { "", "", "", "Subtotal", format.format(recibo.getValorCompra().getSubtotal()) });
	tableModel.addRow(
		new Object[] { "", "", "", "Impuestos", "+ " + format.format(recibo.getValorCompra().getImpuestos()) });
	tableModel.addRow(new Object[] { "", "", "", "Desc. Premium",
		"- " + format.format(recibo.getValorCompra().getDescuentoPremium()) });
	tableModel.addRow(new Object[] { "", "", "", "Desc. Frecuencia",
		"- " + format.format(recibo.getValorCompra().getDescuentoFrecuencia()) });
