...
public class CarritoDAO extends ConexionBD<LibroCarrito> {
    ...
    public void insertarDatos(LibroCarrito libroCarrito) throws SQLException, RuntimeException {
        ...
        try (Connection connection = crearConexion(); PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            RegistroLog.registrarInfo("Se agregaron: " + libroCarrito.getCantidad() + " libros con ISBN: " + libroCarrito.getIsbn_libro() + ",  usuario: " + libroCarrito.getCorreo_usuario() + ".");
    ...
    public ArrayList<Libro> seleccionarRegistros(Libro libroCarrito, String correo) throws SQLException, RuntimeException {
    ...
    public void restarCantidad(LibroCarrito libroCarrito) throws SQLException, RuntimeException {
        ...
        if (libroCarrito.getCantidad() <= 0) {
            RegistroLog.registrarAdvertencia("❗ Se intentó establecer una cantidad no válida: " + libroCarrito.getCantidad());
        ...
        try (Connection connection = crearConexion(); PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            if (filasAfectadas > 0) {
                RegistroLog.registrarInfo("🔄 Cantidad actualizada en el carrito: " + libroCarrito.getCantidad() + " unidades para ISBN " + libroCarrito.getIsbn_libro() + " del usuario " + libroCarrito.getCorreo_usuario());
            } else {
                RegistroLog.registrarAdvertencia("⚠️ No se encontró registro para actualizar con ISBN " + libroCarrito.getIsbn_libro() + " y usuario " + libroCarrito.getCorreo_usuario());
            }
        } catch (SQLException e) {
            RegistroLog.registrarError("❌ Error al restar cantidad para ISBN " + libroCarrito.getIsbn_libro() + " y usuario " + libroCarrito.getCorreo_usuario(), e);
    ...
    public void eliminarRegistro(LibroCarrito libroCarrito) throws SQLException, RuntimeException {
        ...
        try (Connection connection = crearConexion(); PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            if (filasAfectadas > 0) {
                RegistroLog.registrarInfo("🗑️ Libro eliminado del carrito (ISBN: " + libroCarrito.getIsbn_libro() + ", Usuario: " + libroCarrito.getCorreo_usuario() + ").");
            } else {
                RegistroLog.registrarAdvertencia("⚠️ No se encontró el libro para eliminar (ISBN: " + libroCarrito.getIsbn_libro() + ", Usuario: " + libroCarrito.getCorreo_usuario() + ").");
            }
        } catch (SQLException e) {
            RegistroLog.registrarError("❌ Error al eliminar el libro del carrito (ISBN: " + libroCarrito.getIsbn_libro() + ", Usuario: " + libroCarrito.getCorreo_usuario() + ").", e);
